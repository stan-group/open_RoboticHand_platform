...
INDEX_LIFT_DELTA = -12
# Positive wrist angle = flexion, negative wrist angle = extension.
WRIST_FULLY_FLEXED = 30
WRIST_FULLY_EXTENDED = -50
...
def build_fist_pose(base_pose: dict[str, float], wrist_angle: float) -> dict[str, float]:
	fist_pose = dict(base_pose)
	fist_pose.update(
		{
			"thumb_mcp": 50,
			"thumb_abd": 42,
...
def run_hand_motion(duration_s: float, stop_event: threading.Event, hand) -> None:
	...
	try:
		...
		pointing_pose = dict(base_pose)
		lift_pose = build_lift_pose(pointing_pose)
		press_pose = build_press_pose(pointing_pose)
		fist_flexed_pose = build_fist_pose(base_pose, WRIST_FULLY_FLEXED)
		fist_extended_pose = build_fist_pose(base_pose, WRIST_FULLY_EXTENDED)
		open_extended_pose = dict(base_pose)
		open_extended_pose["wrist"] = WRIST_FULLY_EXTENDED

		# Boot sequence: fist with wrist flexed -> fist with wrist extended -> open hand with wrist extended -> pointing pose.
		smooth_move(hand, fist_flexed_pose, num_steps=BOOT_LIFT_STEPS, step_size=BOOT_LIFT_STEP_SIZE)
		if not sleep_with_stop(0.25):
			return
		smooth_move(hand, fist_extended_pose, num_steps=BOOT_LIFT_STEPS, step_size=BOOT_LIFT_STEP_SIZE)
		if not sleep_with_stop(0.25):
			return
		smooth_move(hand, open_extended_pose, num_steps=BOOT_LIFT_STEPS, step_size=BOOT_LIFT_STEP_SIZE)
		if not sleep_with_stop(0.25):
			return
		smooth_move(hand, pointing_pose, num_steps=BOOT_LIFT_STEPS, step_size=BOOT_LIFT_STEP_SIZE)
		if not sleep_with_stop(0.25):
			return
		...
		# Shutdown: return to the pointing pose, then disconnect.
		smooth_move(hand, pointing_pose, num_steps=BOOT_LIFT_STEPS, step_size=BOOT_LIFT_STEP_SIZE)
		if not sleep_with_stop(0.25):
			return
	finally:
...
def connect_hand_with_retry(hand_port: str | None = None):
	...
	if REAL_ORCA_AVAILABLE:
		...
		if hand_port:
			# Ensure model config uses the requested port before creating OrcaHand
			config_path = MODEL_PATH / "config.yaml"
			try:
				_backup_config_path = backup_and_set_config_port(config_path, hand_port)
			except Exception as exc:
			...
			try:
				...
				# Update model config to the detected port before creating OrcaHand
				config_path = MODEL_PATH / "config.yaml"
				try:
					_backup_config_path = backup_and_set_config_port(config_path, detected)
				except Exception as exc:
					print(f"Warning: could not update model config: {exc}")
...
_CONFIG_BACKUP_PATH: str | None = None


def backup_and_set_config_port(config_path: Path, new_port: str) -> str:
	"""Create a backup of the YAML config and update the `port:` value.
	...
	_CONFIG_BACKUP_PATH = str(backup_path)

	# Read and update the 'port:' line (ignore commented lines)
	text = config_path.read_text(encoding="utf-8")
	lines = text.splitlines()
	port_line_re = re.compile(r"^\s*port\s*:\s*(.+)$")
	replaced = False
	for i, line in enumerate(lines):
		...
		if m:
			indent = line[: line.find("port")]
			lines[i] = f"{indent}port: {new_port}"
			replaced = True
			break

	if not replaced:
		# Insert near top after version or baudrate if possible
		insert_at = 0
		for i, line in enumerate(lines[:10]):
			if line.strip().startswith("baudrate") or line.strip().startswith("version"):
				insert_at = i + 1
		lines.insert(insert_at, f"port: {new_port}")

	config_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
	print(f"Wrote port={new_port} into {config_path} (backup: {backup_path})")
	return str(backup_path)


def restore_config_if_backed_up(config_path: Path) -> None:
	global _CONFIG_BACKUP_PATH
	if _CONFIG_BACKUP_PATH and Path(_CONFIG_BACKUP_PATH).exists():
		try:
			shutil.copy2(_CONFIG_BACKUP_PATH, str(config_path))
			print(f"Restored original config from {_CONFIG_BACKUP_PATH}")
			# Optionally remove the backup
			# Path(_CONFIG_BACKUP_PATH).unlink()
		except Exception as exc:
...
def main() -> int:
	...
	try:
		...
		# Restore the model config if we modified it earlier
		try:
			restore_config_if_backed_up(MODEL_PATH / "config.yaml")
		except Exception as exc:
			print(f"Warning: failed to restore model config: {exc}")
